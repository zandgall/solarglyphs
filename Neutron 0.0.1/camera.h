...
public:
	...
	static glm::mat4 genViewMatrix(glm::vec3 Position, glm::vec3 Front, glm::vec3 Up) {
		return glm::lookAt(Position, Position + Front, Up);
	}

	// Smoothly track to a position
	void centerOn(glm::vec3 pos, float deltaTime) {
		float ratio = 0.9*deltaTime;
	...
	void constrain(float left, float up, float right, float down) {
		Position.x = max(min(right, Position.x), left);
		Position.y = max(min(down, Position.y), up);
	}
	...
	void ProcessKeyboard(Camera_Movement direction, float deltaTime) {
		...
		Position.z = max(min(Position.z, 10), 0.2);
	}
