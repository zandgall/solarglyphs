...
void main() {
	...
	if (isGlyph) {
		...
		float alpha = (glyph(c0, v0) + glyph(c1, v1) + glyph(c2, v2) + glyph(c3, v3)) / 4;
		glColor.w *= alpha;

		vec3 normalv = normalize((cammat*normalize(texture2D(plannorm, vec2(1) - TexCoord)*2.0 - 1.0)).xyz);

		float shade = max(dot(normalv, -normalize((wPos).xyz)), 0);

		glColor *= vec4(shade * 0.75 + 0.2, shade * 0.75 + 0.2, shade * 0.7 + 0.2, 1);

		glColor.xyz += max((glColor.b-glColor.g) * shade * pow(max(dot(cammat[2].xyz, reflect(normalize(wPos.xyz), normalv)), 0), 32), 0);
